...
# returns list of all potential moves from other snakes 2 points longer than us
def checkSnakeTiles(state):
    snakeBodyTiles = list()
    ...
    for snake in state["board"]["snakes"]:
        if len(snake["body"]) > len(state["you"]): # TODO: Add a "+ 1" on the right side so our snake only
            ...
            for m in range (0, 4):
                ...
                snakeBodyTiles.append(coords)
    print("potentialTiles:", snakeBodyTiles)
...
def getSafeMoves(state):
    ...
    for move in moves:        
        ...
        print("testing:", nextPoint["x"], nextPoint["y"])
        print(occupied)
        ...
        #if (nextPoint["x"], nextPoint["y"]) in checkSnakeTiles(game_state))
        #safe.append(move)
